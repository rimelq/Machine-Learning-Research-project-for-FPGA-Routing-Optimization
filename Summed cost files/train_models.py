...
def train_models(input_dir="datasets", output_dir="results", target_percentages=[0.5, 0.8, 1.0], lambdas=[0.0001, 0.001, 0.01, 0.1, 1.0]):
    ...
    for target_percentage in target_percentages:
        ...
        # Ridge Regression with multiple lambdas and MultiOutput
        for alpha in lambdas:
            print(f"Training Ridge Regression with alpha={alpha}...")
            ridge_model = MultiOutputRegressor(Ridge(alpha=alpha))
            ridge_model.fit(X_train_scaled, y_train)
            ridge_preds = ridge_model.predict(X_test_scaled)

            ridge_mse = mean_squared_error(y_test, ridge_preds, multioutput="raw_values")
            ridge_r2 = r2_score(y_test, ridge_preds, multioutput="raw_values")
        ...
        rf_mse = mean_squared_error(y_test, rf_preds, multioutput="raw_values")
        ...
        # Linear Regression with MultiOutput
        print("Training Linear Regression...")
        lin_model = MultiOutputRegressor(LinearRegression())
        lin_model.fit(X_train_scaled, y_train)
        lin_preds = lin_model.predict(X_test_scaled)

        lin_mse = mean_squared_error(y_test, lin_preds, multioutput="raw_values")
        lin_r2 = r2_score(y_test, lin_preds, multioutput="raw_values")
